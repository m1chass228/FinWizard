...
namespace {
    int algo_number = 2;
    QString name = "Бирюлёво сводный отчёт";
    std::string dir = "Бирюлево";
    std::string filename = "Бирюлево_сводный_отчет.xlsx";
    QString pythonScript = getAlgorithmScriptPath("birulyovo_svodni/biruovo_svodni.py");
}
...
void algo2()
{
    ...
    try {
        ...
        // 3. Папки с данными
        QString output_xlsx = QString::fromStdString((fs::path(get_dir_on_desktop()) / filename).string()); // название конечного
...
REGISTER_ALGORITHM(algo_number, name, QString::fromStdString(dir), algo2);
